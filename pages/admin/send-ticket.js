...
export default function SendTicket() {
  ...
  const sendData = async ({ email, collegeid }) => {
    setLoading(true);
    console.log(email, collegeid);

    let encode_string = `${collegeid}`;
    console.log({ encode_string });
    try {
      let res = await fetch("/api/order/ticket/send", {
        method: "POST",
        headers: {
          Accept: contentType,
  ...
  const handleTicketSend = () => {
    console.log(data);

    let fields = ["collegeid", "email"];
  ...
  return (
    <Layout title={"ADMIN Send Ticket"} access={"admin"}>
            ...
            <div className="md:col-span-1">
              <div className="px-4 sm:px-0">
                ...
                <p className="mt-1 text-sm text-gray-600">
                  Send Tickets to users for Special Event
                </p>
              </div>
            </div>
